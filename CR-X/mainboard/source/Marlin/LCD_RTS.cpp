...
void RTSSHOW::RTS_Init()
{
	...
	#if FAN_COUNT > 0
		for (uint8_t i = 0; i < FAN_COUNT; i++) fanSpeeds[i] = 0;//turn 0ff fans
	#endif
	...
	for(int j = 0;j < 30;j++)	//clean screen.
		RTS_SndData(0,Printfilename+j);
	for(int j = 0;j < 20;j++)	//clean filename
		RTS_SndData(0,Choosefilename+j);
	for(int j = 1;j <= MaxFileNumber;j++)
		RTS_SndData(5,FilenameIcon+j);
...
void RTSSHOW::RTS_HandleData()
{
	...
	case PrintChoice:
		...
		else if(recdat.addr == Resumeprint)
		{				
			char pause_str_Z[16];
			memset(pause_str_Z, 0, sizeof(pause_str_Z));
			dtostrf(pause_z, 3, 2, pause_str_Z);
			sprintf_P(commandbuf, PSTR("M140 S%i"), tempbed);
	...
	case TempControl:
		...
		else if(recdat.data[0] == 6)	//ABS mode
		{
			...
			fanSpeeds[0] = PREHEAT_2_FAN_SPEED;
	...
	case Setting:
		if(recdat.data[0] == 1)	//Bed leveling
		{
	...
	case XYZEaxis:
		...
		else if(recdat.addr == AutoZero)
		{
			if(recdat.data[0] == 3)	//autohome
			{
				...
				RTS_SndData(0, DisplayXaxis);
				RTS_SndData(0, DisplayYaxis);
				RTS_SndData(0, DisplayZaxis);
				RTS_SndData(10,FilenameIcon);
			}
			else
			{
...
{		
	...
	if(ms > next_rts_update_ms && InforShowStatus)
	{
		...
		{
			if (previous_cmd_ms != 0)		//need to optimize
			{
			...
			if(NozzleTempStatus[0])	//statuse of loadfilement and unloadfinement when temperature is less than
			{
				unsigned int IconTemp;
				...
				if(thermalManager.current_temperature[0] >= thermalManager.target_temperature[0])
				{
					NozzleTempStatus[1] = 0;
